...
class WarGamePlay:GamePlay, GamePlayProtocol {
    ...
    func makePlayerViewAndController(initializedPlayer:Player?, playerNumber:Int, playerSessionId:String, isPlayerLocal:Bool, playerTop:CGFloat, playerName:String) {
        ...
        if let player = initializedPlayer {
            ...
            os_log("making player %d: model exists", log:self.log, type:.debug, initializedPlayerNumber)
            ...
            os_log("making player %d: model does not exist", log:self.log, type:.debug, playerNumber)
